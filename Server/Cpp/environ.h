...
SUBJECT       : Defines the compilation and the target environments.
PROGRAMMER    : (C) Copyright 2005 by Peter C. Chapin

This file contains settings that define the environment in which the
program was compiled and the environment where it runs. This file should
be included into source files that need to distinguish one environment
from another. Conditional compliation directives can then be used to
select appropriate code for each environment.

I find it easier to use the symbols defined in this file than it is to
use the symbols defined by the various individual compilers. The symbols
here are more consistent and more natural. However, it is my intention
for this file to use the compiler specific symbols to automatically set
appropriate values for the symbols defined here whenever possible.

Each symbol defined here is prefixed with 'e' (for "environment"). This
is done because several of the symbol names are fairly generic and they
tend to collide with similarly named symbols in other libraries.
...
#define eOPENWATCOM  8  // OpenWatcom

// Choose your compiler! This file can autodetect all of the compilers
// mentioned above. If the compiler can't be autodetected it will
// default to eVANILLA.
...
// It might make sense to re-encode the compiler version also.
...
#define eWIN32   7  // WinNT/2000/XP only. Win95/98/Me are obsolete.

// Choose your operating system! In most cases this file can autodetect
// the operating system from the compiler that is being used. If that is
// not the case, you will have to specify the operating system if it
// matters. There is no default.
...
// Assume gcc is on Unix. This assumption should be lifted eventually.
#if eCOMPILER == eGCC
#define eOPSYS ePOSIX
#endif
...
// OpenWatcom supports a variety of different systems.
...
// Choose your GUI. This file does not currently autodetect any GUI. The
// default GUI is eNONE. Note that if the operating system is eMAC its
// native GUI can be assumed. That is not the case for the other
// operating systems since they support text mode applications as an
// option.
...
// When writing a multithreaded program, there are additional issues
// that must be considered. The symbol eMULTITHREADED will be defined in
// all such cases. This file can auto-detect this feature in some but
// not all cases.
//
// Note that if eMULTITHREADED is defined when eOPSYS is ePOSIX, Posix
// threads are implied. If eOPSYS is eWIN32, Windows threads are implied.
//
