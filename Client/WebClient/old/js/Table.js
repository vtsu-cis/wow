...
Table.prototype.generateHTML = function(divObj, delimiter)
{
	with (this) {
		...
		line = "<table border=\"1\" width=\"100%\"><tr>";
		for (var i = 0; i < columns.length; i++) {
		    if(columnSorted == columns[i]){  //column is already being sort, so sort in the opposite direction
			   if(sortOrder == ASC){         //sorted ascending 
			      line += "<th onMouseOver = \"javascript:highlight(this)\" onMouseOut = \"javascript:unHighlight(this)\" onClick=\"javascript:doSort('" + columns[i] + "')\"><a href = \"#\" class = \"outpt\">" + columns[i] + " <img src = \"img/up_arrow.gif\"></a></th>";
			   }
			   else{						//sorted ascending
			      line += "<th onMouseOver = \"javascript:highlight(this)\" onMouseOut = \"javascript:unHighlight(this)\" onClick=\"javascript:doSort('" + columns[i] + "')\"><a href = \"#\" class = \"outpt\">" + columns[i] + " <img src = \"img/down_arrow.gif\"></a></th>";
			   }
			}
			else{							//columns NOT sorted
			   line += "<th onMouseOver = \"javascript:highlight(this)\" onMouseOut = \"javascript:unHighlight(this)\" onClick=\"javascript:doSort('" + columns[i] + "')\"><a href = \"#\" class = \"outpt\">" + columns[i] + "</a></th>";
			} 
		...
		if (rows.length == 0) {
			line += "<tr><td>No results.</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td></tr>";
		}
...
{
  element.style.background = "#73880A";
}

function unHighlight(element)
{
  element.style.background = "#CDEB8B";
}